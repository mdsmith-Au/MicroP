...
void printToAddress(char* string, int length, int address);
...
void sendCommand(uint8_t command) {
	...
	// Reset has REG_SEL = 0, Read/Write = 0, Enable 0 at start
...
void sendASCII(uint8_t data) {
	// ASCII write has REG_SEL = 1, Read/Write = 0, Enable 0 at start
	GPIO_ResetBits(LCD_GPIO_BANK, LCD_RW | LCD_EN);
	GPIO_SetBits(LCD_GPIO_BANK, LCD_REG_SEL);
	
	// Place command on data lines, one bit at a time
	GPIO_WriteBit(LCD_GPIO_BANK, LCD_DATA_0, data & BIT1);
	GPIO_WriteBit(LCD_GPIO_BANK, LCD_DATA_1, data & BIT2);
	GPIO_WriteBit(LCD_GPIO_BANK, LCD_DATA_2, data & BIT3);
	GPIO_WriteBit(LCD_GPIO_BANK, LCD_DATA_3, data & BIT4);
	GPIO_WriteBit(LCD_GPIO_BANK, LCD_DATA_4, data & BIT5);
	GPIO_WriteBit(LCD_GPIO_BANK, LCD_DATA_5, data & BIT6);
	GPIO_WriteBit(LCD_GPIO_BANK, LCD_DATA_6, data & BIT7);
	GPIO_WriteBit(LCD_GPIO_BANK, LCD_DATA_7, data & BIT8);
